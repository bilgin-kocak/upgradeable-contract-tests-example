...
describe('Contract Version 2 test', function () {
  ...
  beforeEach(async function () {
    ...
    const OurUpgradeableNFT1 = await ethers.getContractFactory(
      'OurUpgradeableNFT1'
    );
    const OurUpgradeableNFT2 = await ethers.getContractFactory(
      'OurUpgradeableNFT2'
    );
  ...
  it('Only Owner can mint NFTs', async function () {
    await expect(
      upgradedContract.connect(addr1).safeMint(addr1.address, 'Test NFT 2')
